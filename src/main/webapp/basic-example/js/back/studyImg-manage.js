...
function bindEvents() {
    $('#btnNew').click(create);
    $('#btnDelete').click(del);//批量删除暂未实现
    $('#chkSelectAll').click(selectAll);
}
function create() {
    window.location.href = 'create-studyimg.html';
}
function del() {
...
function loadData() {
    var filter = encodeURIComponent(JSON.stringify({"kind": 2}));
...
function edit(evt) {
    ...
    window.location.replace('create-studyimg.html?id=' + id);
}