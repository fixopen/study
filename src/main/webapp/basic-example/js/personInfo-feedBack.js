...
function genericLoadData(kind) {
    ...
    url = '/api/users/' + userId + '/feedbacks?filter=' + filter;
...
//    var goPageType;
    ...
    var template = g.getTemplate('feedback-template')
...
function checkReply(evt) {
    ...
    window.location.replace('personInfo-feedBack-detail.html?id=' + id);
}
function delRow(evt) {
    var id = evt.target.attributes['rowid'].value;
    ...
    g.deleteData('/api/users/' + userId + '/feedbacks/' + id, function (data) {
        if (data.state == 200) {
