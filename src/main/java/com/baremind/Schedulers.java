...
public class Schedulers {
	...
	public Response getThisWeekScheduler(@CookieParam("sessionId") String sessionId,
		...
		if (JPAEntry.isLogining(sessionId)) {
			...
			// filterObject = new Gson().fromJson(rawFilter, new
	 ...
	 * (JPAEntry.isLogining(sessionId)) { Map<String, Object> filterObject =
	 * null; if (filter != "") { String rawFilter = URLDecoder.decode(filter);
	 * filterObject = new Gson().fromJson(rawFilter, new TypeToken<Map<String,
	 * Object>>() { }.getType()); } List<Scheduler> schedulers =
	 * JPAEntry.getList(Scheduler.class, filterObject); result = Response.ok(new
	 * Gson().toJson(schedulers)).build(); } else { result =
	 * Response.status(404).build(); } return result; }
	 */
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response updateScheduler(@CookieParam("sessionId") String sessionId, @PathParam("id") Integer id,
			Scheduler scheduler) {
		Response result = Response.status(401).build();
		if (JPAEntry.isLogining(sessionId)) {
